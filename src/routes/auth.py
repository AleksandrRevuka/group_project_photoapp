...
async def user_logout(
    ...
    return {"message": messages.get_message("TOKEN_REVOKED")}
...
async def confirmed_email(token: str, db: AsyncSession = Depends(get_db)) -> dict:
    ...
    if user.confirmed:
        return {"message": messages.get_message("EMAIL_ALREADY_CONFIRMED")}
    await repository_users.confirmed_email(email, db)
    return {"message": messages.get_message("EMAIL_CONFIRMED")}
...
async def request_email(
    ...
    if user and not user.confirmed:
        ...
        return {"message": messages.get_message("CHECK_EMAIL_CONFIRMATION")}
    else:
        return {"message": messages.get_message("EMAIL_ALREADY_CONFIRMED")}
...
async def forgot_password(
    ...
    if user:
        ...
        background_tasks.add_task(send_email, user.email, user.username, str(request.base_url), subject, template)
        return {"message": messages.get_message("PASSWORD_RESET_SEND_REQUEST")}

    return {"message": messages.get_message("USER_NOT_FOUND_PROVIDED_EMAIL")}
