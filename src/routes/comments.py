...
async def update_comment(
    ...
    if comment is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=messages.COMMENT_NOT_CREATED)
    return comment
...
async def comments_to_photo(
    ...
    if not comments:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=messages.COMMENTS_NOT_FOUND)
    return comments
